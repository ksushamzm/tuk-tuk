...
const CategoryTemplate4: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="w-full bg-white font-roboto overflow-hidden">
      ...
      {/* Mobile Row 1: Star Left / Text Right (blocks[3].text) */}
      <Link to={`/article/${blocks[3].id || data.id}`} className="md:hidden p-[14px] border-b-2 border-black bg-white flex flex-row relative min-h-[220px] hover:bg-thai-yellow/5 transition-colors group">
        <Star className="bottom-4 left-4" />
        <div className="w-1/2 pr-2"></div>
        <div className="w-1/2 flex flex-col justify-end items-end pb-[14px]">
          <p className="text-[14px] font-normal italic leading-[1.3] text-black w-full mb-4">
      ...
      {/* Mobile Row 2: Photo + Caption (blocks[4]) */}
      <div className="relative overflow-hidden bg-white border-b-2 md:border-b-0 md:border-r-2 border-black md:hidden">
        ...
        <img src={blocks[4].image} className="w-full aspect-[4/3] object-cover" alt="" />
        <div className="block p-[14px] h-[68px] flex items-center justify-between border-t-2 border-black bg-white transition-colors">
          <p className="text-[12px] font-normal italic leading-tight uppercase">{fixHangingPrepositions(blocks[4].title)}</p>
      ...
      {/* Mobile Row 4: Empty Left / Mountain Right (blocks[5]) */}
      <div className="md:hidden flex h-[304px] border-b-2 border-black">
        <div className="w-1/2 border-r-2 border-black bg-white"></div>
        <div className="w-1/2 relative overflow-hidden flex flex-col justify-end bg-white transition-colors">
          ...
          <div className="relative z-10 p-[14px] pb-8 flex flex-col items-center">
            ...
            <div className="self-end mr-2 transition-transform"><ArrowIcon /></div>
          </div>
        </div>
      </div>

      {/* Mobile Row 5: TEST Block (blocks[0]) */}
      <div className="md:hidden flex h-[220px] border-b-2 border-black relative">
        <Star className="top-2 left-1/2 -translate-x-1/2 z-20" />
        <div className="w-1/2 p-[14px] flex flex-col justify-between border-r-2 border-black relative bg-white transition-colors">
          <div className="flex flex-col scale-50 origin-left">
            <TestLogo />
          </div>
          <div className="flex flex-col items-start mt-4">
            <div className="mb-2 transition-transform"><ArrowIcon /></div>
      ...
      {/* Mobile Row 6: Mountain Left (blocks[2]) / Empty Right */}
      <div className="md:hidden flex h-[304px] border-b-2 border-black">
        <div className="w-1/2 relative overflow-hidden flex flex-col justify-end border-r-2 border-black bg-white transition-colors">
          <Star className="top-2 right-2" />
          <div className="absolute inset-x-0 bottom-0 flex items-end">
            <ThaiMountainPattern className="w-full h-auto" />
          </div>
      ...
      {/* Row 1: Mountain Left / Green + Text Right (732px) */}
      <div className="hidden md:flex flex-col md:flex-row border-b-2 border-black relative md:h-[50.8vw]">
        <div className="md:w-[33.47%] border-b-2 md:border-b-0 md:border-r-2 border-black flex flex-col relative overflow-hidden bg-white h-[304px] md:h-full justify-end transition-colors">
          <Star className="top-4 right-4" />
          <div className="absolute inset-0 flex items-end">
            <ThaiMountainPattern className="w-full h-auto" />
          </div>
          <div className="relative z-10 p-[14px] md:p-[1.25vw] flex flex-col items-center">
            <p className="text-[14px] md:text-[2.2vw] font-normal italic leading-tight text-center mb-4 md:mb-8 bg-white px-2">
              {fixHangingPrepositions(blocks[2].title)}
            </p>
            <div className="self-end mr-2 transition-transform">
              <ArrowIcon />
            </div>
          </div>
        </div>
        ...
        <div className="md:w-[66.53%] flex flex-col">
          {/* Green Ornament */}
          <GreenSection />
          <Link to={`/article/${blocks[3].id || data.id}`} className="flex-1 p-[14px] md:p-[3vw] flex flex-col md:flex-row relative bg-white min-h-[300px] border-t-2 border-black hover:bg-thai-yellow/5 transition-colors group cursor-pointer">
            <div className="md:w-1/2 flex flex-col justify-between mb-6 md:mb-0 relative">
              ...
              <div className="absolute bottom-4 left-4 md:bottom-[0vw] md:left-[0vw]">
      ...
      {/* Row 2: Photo + Caption Left / Yellow Section / Mountain Right (737px) */}
      <div className="hidden md:flex flex-col md:flex-row border-b-2 border-black relative md:h-[51.1vw]">
        ...
        <div className="md:w-[56.81%] border-b-2 md:border-b-0 md:border-r-2 border-black flex flex-col relative overflow-hidden bg-white transition-colors">
          <Star className="top-4 right-4" />
          ...
          <div className="p-[14px] md:p-[1.25vw] h-[68px] md:h-[9.7vw] md:min-h-[140px] flex flex-col justify-center border-t-2 border-black bg-white">
            <div className="flex justify-between items-center">
              <p className="text-[12px] md:text-[2.2vw] font-normal italic leading-tight max-w-[80%] uppercase md:normal-case">
        ...
        <div className="md:w-[33.33%] flex flex-col relative bg-white h-[304px] md:h-full overflow-hidden transition-colors">
          <Star className="top-4 right-4" />
      ...
      <div className="hidden md:flex flex-col md:flex-row border-b-2 border-black relative md:h-[37.1vw]">
        ...
        <div className="flex flex-col md:flex-row h-full md:w-[66.53%] md:border-r-2 border-black transition-colors">
          <div className="w-full p-[14px] md:p-[3vw] flex flex-col justify-between relative bg-white h-[220px] md:h-full justify-end">
            <Star className="top-2 right-2 md:hidden" />
            <div className="flex flex-col scale-50 md:scale-100 origin-left mt-2 md:mt-0">
              <TestLogo />
            </div>
            <div className="flex flex-col md:flex-row md:items-end justify-between mt-auto mb-[2vw]">
              <div className="md:hidden mb-2 transition-transform">
                <ArrowIcon />
              </div>
              <p className="text-[14px] md:text-[2.2vw] font-normal italic leading-tight max-w-[200px] md:max-w-sm">
                {fixHangingPrepositions(blocks[0].title)}
              </p>
              <div className="hidden md:block ml-auto pr-6 transition-transform">
                <ArrowIcon />
              </div>
            </div>
          </div>
      ...
      <div className="hidden md:flex flex-col md:flex-row border-b-2 border-black relative md:h-[37.1vw]">
        ...
        <div className="md:w-[66.53%] p-[14px] md:p-[3vw] flex flex-col md:flex-row relative overflow-hidden bg-white">
          ...
          <div className="md:w-1/2 flex flex-col justify-center items-end relative md:pt-[10%]">
            <div className="block w-full h-full transition-colors flex flex-col justify-between">
              <p className="text-[12px] md:text-[2vw] font-normal italic leading-[1.3] text-black w-full">
                {fixHangingPrepositions(blocks[1].text || '')}
              </p>
              <div className="mt-6 md:mt-auto pt-4 md:pt-8 flex justify-end transition-transform">
                <ArrowIcon />
              </div>
            </div>
          </div>
...
export default CategoryTemplate4;
